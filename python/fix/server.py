...
class FIXClient:
    ...
    def send_new_order(self, symbol, side, order_type, quantity, price=None):
        ...
        order.append_pair(60, datetime.now().strftime("%Y%m%d-%H:%M:%S.%f")[:-3])  # TransactTime
    ...
    def send_cancel_request(self, original_order_id, symbol, side):
        ...
        cancel.append_pair(52, datetime.now().strftime("%Y%m%d-%H:%M:%S.%f")[:-3])
        ...
        cancel.append_pair(60, datetime.now().strftime("%Y%m%d-%H:%M:%S.%f")[:-3])  # TransactTime
